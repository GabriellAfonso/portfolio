...
let chatRoomInput = document.getElementById('sender-input');
...
function openRoomMobile(room_id) {
    openRoom(room_id); // já existente
    if (window.innerWidth <= 768) {
        document.getElementById('user-controller').style.display = 'none';
...
function returnToList() {
    ...
    document.getElementById('user-controller').style.display = 'flex';
}
...
window.addEventListener('resize', function () {
    ...
    const heightChanged = Math.abs(window.innerHeight - lastWindowHeight) > 200;
    ...
    if (widthChanged || heightChanged) {
        // Só executa se realmente mudou de tamanho significativamente
        if (window.innerWidth > 768) {
            document.getElementById('user-controller').style.display = 'flex';
            document.getElementById('chat').style.display = 'block';
        } else {
            if (activeRoomID == 0) {
                document.getElementById('user-controller').style.display = 'block';
                document.getElementById('chat').style.display = 'none';
            } else {
                document.getElementById('user-controller').style.display = 'none';
                document.getElementById('chat').style.display = 'block';
